...
`security-review/code-analyzer-cleancode.csv` and `security-review/code-analyzer-security.csv` are the human-curated baseline of grandfathered findings. **The AI agent must never edit, delete, or commit these files** — they get regenerated by `scripts/create-scratch-org.sh` and humans decide what to commit. CI compares the post-change scan against this baseline; any new rows trigger a fix-up loop. The agent's job is to fix new findings, not to hide them by updating the baseline.
